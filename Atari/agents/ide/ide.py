...
class IDE():
    ...
    def predict(self, state, directed_exploration=False):
        if not directed_exploration: #Choose greedily
            ...
            if self.logging:
                self.logger.add_scalar('Epistemic_Uncertainty', uncertainties_epistemic[action].sqrt(), self.timestep)
                self.logger.add_scalar('Aleatoric_Uncertainty', uncertainties_aleatoric[action].sqrt(), self.timestep)
