...
class CNNDeepmind_k_head(nn.Module):

    def __init__(self, observation_space, n_outputs, n_heads=10, width=512,weight_scale=np.sqrt(2),):
        # CNN architechture of DeepMind described in https://storage.googleapis.com/deepmind-media/dqn/DQNNaturePaper.pdf :
        ...
        # This is followed by a third convolutional layer that convolves 64 filters of 3 3 3 with stride 1 followed by a rectifier.
        # The final hidden layer is fully-connected and consists of 512 rectifier units.
        # We use the trick to compute in parallel the heads in 
        # https://stackoverflow.com/questions/58374980/run-multiple-models-of-an-ensemble-in-parallel-with-pytorch/58389075#5838907
        ...
        self.output = nn.Sequential(
            nn.Conv1d(64 * 7 * 7 * n_heads, width * n_heads, kernel_size=1, groups=n_heads),
            nn.ReLU(),
    ...
    def forward(self, obs):
        if len(obs.shape) != 4:
            obs = obs.unsqueeze(0)
        out = obs.permute(0,3,1,2)
        out = out/255
        out = self.conv(out)
        out = out.view(obs.size(0), -1) # Flatten the conv output new shape : B x 64*7*7
        out = out.repeat(1, self.n_heads).unsqueeze(2)
